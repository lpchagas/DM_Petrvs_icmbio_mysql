...
> [docs/07.2-status_artefatos_pgd_icmbio.md](07.2-status_artefatos_pgd_icmbio.md).
...
| `periodo_status` (encerrado/em_andamento) | Todos os 12 | Calculado em Python (`build_periods_pe/pt`) comparando `data_fim` do período a `date.today()` — nunca vem do banco |
